...
/**
 * 
 * La clase controlador del la funcionalidad del panel (TripsAndEventsPanel).
 * 
 * Maneja las interacciones en el panel de viajes y eventos Gestiona la
 * presentacion de viajes y eventos asociados, ademas de permitir la creacion,
 * eliminacion y visualizacion de estos elementos.
 */
public class TripsAndEventsPanelController {
	...
	/**
	 * Constructor que inicializa el controlador del panel TripsAndEventsPanel.
	 */
	...
	/**
	 * Genera una oferta en formato de texto y la guarda en un archivo
	 */
	...
	/**
	 * Genera una descripcion de las ofertas de los viajes asociados a la agencia
	 * 
	 * @return Retorna la cadena con la descripcion de las ofertas
	 */
	...
	/**
	 * Genera una lista de los eventos asociados a un viaje especifico
	 * 
	 * @param tripId El identificador del viaje
	 * @return Retorna la cadena con la descripcion de los eventos del viaje
	 */
	private String eventsTextGenerator(String tripId) {
		...
		if (null != planViajes) {
			for (PlanViaje planViaje : planViajes) {
				...
				String descEvento = "    " + "Descripción: " + "\n" + "                " + "Tipo de Evento:   "
						+ planViaje.getTipoEvento() + "\n" + "                " + "Trayecto     :    "
						+ planViaje.getTrayecto() + "\n" + "                " + "Fecha Ida:        "
						+ String.valueOf(planViaje.getIda().getFechaIda()) + "\n" + "                "
						+ "Aerolínea Ida:    "
						+ aerolineaDAO.getAirlineByCod(planViaje.getIda().getAerolinea()).getNameAirline() + "\n";
				if (planViaje.getTrayecto().equalsIgnoreCase("Ida-Vuelta")) {
					descEvento += "                " + "Fecha Vuelta:    "
							+ String.valueOf(planViaje.getVuelta().getFechaIda()) + "\n" + "                "
							+ "Aerolínea Vuelta: "
							+ aerolineaDAO.getAirlineByCod(planViaje.getVuelta().getAerolinea()).getNameAirline() + "\n"
							+ "                " + "Precio:           "
							+ String.valueOf(planViaje.getIda().getPrecio() + planViaje.getVuelta().getPrecio()) + "\n";
				} else {
					descEvento += "                " + "Precio:           "
							+ String.valueOf(planViaje.getIda().getPrecio()) + "\n";
				}
		...
		if (null != alojamientos) {
			for (Alojamiento alojamiento : alojamientos) {
				...
				String descEvento = "    " + "Descripción: " + "\n" + "                " + "Tipo de Evento:     "
						+ alojamiento.getTipoEvento() + "\n" + "                " + "Nombre del Hotel:   "
						+ alojamiento.getNomHotel() + "\n" + "                " + "Tipo de Dormitorio: "
						+ alojamiento.getTipoHab() + "\n" + "                " + "Ciudad:             "
						+ alojamiento.getCiudad() + "\n" + "                " + "Fecha de Entrada:   "
						+ String.valueOf(alojamiento.getFechaEnt()) + "\n" + "                " + "Fecha de Salida:    "
						+ String.valueOf(alojamiento.getFechaSal()) + "\n" + "                " + "Precio:             "
						+ String.valueOf(alojamiento.getPrecio()) + "\n";
				ret += eventTitle + descEvento;
			}
		}
		...
		if (null != actividades) {
			for (Actividad actividad : actividades) {
				...
				String descEvento = "    " + "Descripción: " + "\n" + "                " + "Tipo de Evento:   "
						+ actividad.getTipoEvento() + "\n" + "                " + "Descripción:      "
						+ actividad.getDescripcion() + "\n" + "                " + "Fecha:             "
						+ String.valueOf(actividad.getFecha()) + "\n" + "                " + "Precio:           "
						+ String.valueOf(actividad.getPrecio()) + "\n";
				ret += eventTitle + descEvento;
			}
		}
	...
	/**
	 * Muestra los viajes disponibles en la tabla
	 */
	...
	/**
	 * Muestra los eventos asociados a un viaje en particular cuando se selecciona
	 * un viaje de la tabla
	 */
	...
	/**
	 * Muestra los eventos especificos de un viaje seleccionado
	 * 
	...
	/**
	 * Maneja la accion cuando se hace clic en el boton para crear un nuevo evento
	 */
	...
	/**
	 * Maneja la accion cuando se hace clic en el boton para crear un nuevo viaje
	 */
	...
	/**
	 * Maneja la accion cuando se hace clic en el boton de cerrar sesion
	 */
	...
	/**
	 * Maneja la accion cuando se hace clic en el boton de eliminar un viaje
	 */
	...
	/**
	 * Maneja la accion cuando se hace clic en el botón de eliminar un evento
	 */
	private void deleteEventButton() {
		...
		if (selectedRow != -1) {
			...
			ArrayList<Alojamiento> hotels = alojamientoDAO.getAllAlojamientos();
			ArrayList<PlanViaje> plans = planViajeDAO.getAllPlanViajes();
			ArrayList<Actividad> activities = actividadDAO.getAllActividades();
			for (Alojamiento hotel : hotels) {
				if (hotel.getIdEvento().equalsIgnoreCase(idEvento)) {
					alojamientoDAO.deleteAlojamientoById(idEvento);
					break;
				}
			}
			for (PlanViaje plan : plans) {
				if (plan.getIdEvento().equalsIgnoreCase(idEvento)) {
					planViajeDAO.deletePlanViajeById(idEvento);
					break;
				}
			}
			for (Actividad activity : activities) {
				if (activity.getIdEvento().equalsIgnoreCase(idEvento)) {
